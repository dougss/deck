...
export function DeleteSessionDialog({
  ...
  const isAttached = session.ptyId !== null

  async function handleDelete(): Promise<void> {
    setSubmitting(true)
    setError(null)
    try {
      await window.deck.session.delete(session.id)
      onClose()
    } catch (err) {
  ...
  return (
    ...
    >
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Delete &ldquo;{session.name}&rdquo;?</DialogTitle>
          <DialogDescription>
            {isAttached
        ...
        <div className="px-6 pb-2">
          {isAttached ? (
            <p className="font-body text-[13px] text-op-zinc-400 leading-relaxed">
              The terminal process will be terminated and the session will be permanently deleted.
            </p>
          ) : (
            <p className="font-body text-[13px] text-op-zinc-400 leading-relaxed">
